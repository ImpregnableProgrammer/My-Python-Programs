...
_tg_turtle_functions = ['back', 'backward', 'begin_fill', 'begin_poly', 'bk',
        ...
        'write', 'xcor', 'ycor']
_tg_utilities = ['write_docstringdict', 'done', 'savefirst', 'save', 'savior', 'background', 'bgresize', 'bgflip', 'bgmirror', 'bgrotate', 'originalBG', 'clearbg', 'PicCounter', 'TurtleShape', 'TurtleImageResize', 'flippic', 'mirror', 'rotatePic', 'original23']           
...
def savefirst():
   try:
      ...
      global hen
      hen = TK.filedialog.asksaveasfilename()
      print(hen)
...
def TurtleShape(iop = None):
   ...
   pictures.append(im)
   edited.clear()
##   print(os.path.exists(ccn + "/Outfile.gif"))
##   if not os.path.exists(ccn + "/Outfile.gif"):
   if len(dwon) > 0:
       PicCounter()
       im.save(tmp2 + "/Outfile" + str(PicCounter.counter) + '.gif', "GIF")
...
dimp = collections.deque()
art = collections.deque()

skomp = collections.deque()
...
##                pass
        try:
            ...
            ep = art.popleft()
            imu = Image.open(ep)
            imu.save(tmp2 + "/Outfile" + str(PicCounter.counter) + ".gif")
            register_shape(tmp2 + "/Outfile" + str(PicCounter.counter) + ".gif")
            shape(tmp2 + "/Outfile" + str(PicCounter.counter) + ".gif")
        except:
            tsd = i.recieveshape()
...
##            update()
        j = i.getfunction()
        if j.__name__ == "turtleone" or j.__name__ == "buttony":
            Count()
            pictures.extend(hfl)
...
##             turtleone(tmp2 + "/Outfile" + str(PicCounter.counter) + ".gif")
            print(str((len(pictures)) - Count.counter))
            if (len(pictures) - Count.counter) == 1:
                 shape(tmp2 + "/Outfile.gif")
                 update()
            else:
                 shape(tmp2 + "/Outfile" + str((len(pictures)) - Count.counter) + ".gif")
                 update()
            print("Puppy")
            dwon.append("1")
...
def lencounter():
   if not hasattr(lencounter, "counter"):
       lencounter.counter = -1
   lencounter.counter += 1

def savefunc():
   global snf
   snf = filedialog.asksaveasfilename()
   ...
   if len(pictures) > 0:
       ...
       lencounter()
       sng = open(snf + "/AutoFile" + str(lencounter.counter) + ".txt", "w")
    ##       for c in pictures:
    ##           fnh.write("PicCounter()\n")
    ##           fnh.write("c.save(tmp2 + '/Output' + str(PicCounter.counter) + '.gif')\n")
    ##           fnh.write("snf = Image.open(tmp2 + '/Output' + str(PicCounter.counter) + '.gif')\n")
    ##           fnh.write("pictures.append(snf)\n")
       for s in pictures:
           sng.write(str(s) + '\n')
       sng.close()
       nip = open(snf + "/AutoFile" + str(lencounter.counter) + ".txt", "r")
       contents2 = nip.read()
       print(contents2)
       print("Hall!!!")
   else:
       pass
   fnh = open(snf + "/" + "Commands.txt", "w")
   fnh.write("def buttony():\n")
   fnh.write("\tPicCounter()\n")
   fnh.write("\tmanipulateimage.config(state = NORMAL)\n")
   fnh.write("\tflipButton.config(state = NORMAL)\n")
   fnh.write("\tmirrorButton.config(state = NORMAL)\n")
   fnh.write("\toriginalButton.config(state = NORMAL)\n")
   fnh.write("\tresetturtle.config(state = NORMAL)\n")
   fnh.write("\trotateButton.config(state = NORMAL)\n")
   fnh.write("\tmif = filename\n")
   fnh.write("\tif os.path.isfile(tmp2 + '/Outfile.gif'):\n")
   fnh.write("\t\tmif.save(tmp2 + '/Outfile' + str(PicCounter) + '.gif', 'GIF')\n")
   fnh.write("\t\tTurtleShape(tmp2 + '/Outfile' + str(PicCounter) + '.gif')\n")
   fnh.write("\t\tprint('Shaped!')\n")
   fnh.write("\telse:\n")
   fnh.write("\t\tmif.save(tmp2 + '/Outfile.gif', 'GIF')\n")
   fnh.write("\t\tTurtleShape(tmp2 + '/Outfile.gif')\n")
   fnh.write("\t\tprint('Shaped2!')\n")
   for item in den:
       if item == "TurtleImageResize":
           fnh.write("PicCounter()\n")
           fnh.write("lunt = shape()\n")
           fnh.write("pool = Image.open(lunt)\n")
           fnh.write("guip = picwidth.popleft()\n")
           fnh.write("picwidth.append(guip)\n")
           fnh.write("hyp = picheight.popleft()\n")
           fnh.write("picheight.append(hyp)\n")
           fnh.write("swim = pool.resize((int(guip), int(hyp)), Image.ANTIALIAS)\n")
           fnh.write("pictures.append(swim)\n")
           fnh.write("edited.append(swim)\n")
           fnh.write("swim.save(tmp2 + '/Outfile' + str(PicCounter.counter) + '.gif', 'GIF')\n")
           fnh.write("register_shape(tmp2 + '/Outfile' + str(PicCounter.counter) + '.gif')\n")
           fnh.write("shape(tmp2 + '/Outfile' + str(PicCounter.counter) + '.gif')\n")
           fnh.write("update()\n")
       elif item == "turtleone":
           fnh.write("draw(buttony)\n")
##           fnh.write("\tregister_shape(tmp2 + '/Outfile.gif')\n")
##           fnh.write("\tshape(tmp2 + '/Outfile.gif')\n")
       elif item == "<lambda>":
           fnh.write("draw(clearer)")
       else:
           fnh.write("draw(%s)\n" % item)
   fnh.close()
   kip = open(snf + "/" + "Commands.txt", "r")
   contents = kip.read()
   print(contents)

def openit():
   ghj = filedialog.askopenfilename()
   global ccn
   ccn = os.path.dirname(os.path.realpath(str(ghj)))
   print(ccn)
##   for filename in glob.glob(ccn + "/Outfile.gif"):
   global filename
   filename = Image.open(ccn + "/Outfile.gif", "r")
   try:
       print(tmp2)
   except NameError:
       maketemp()
...
def Options():
   ...
   global original
   original = Button(text = "Set to Default Dimensions", command = OriginalLetters)
   try:
       rit = width345.pop()
       width345.append(rit)
   except:
   ...
   global mno
   mno = Menubutton(text = "Previous heights")
   mno.pack(side = "left")
   ...
   global hjk
   hjk = Menubutton(text = "Previous widths")
   hjk.pack(side = "left")
...
def options2():
  ...
  bgButton = Button(image = bgpicphoto, command = bgone)
  ...
  global bgflipbutton
  bgflipbutton = Button(image = bgflipphoto, command = bgflip, state = DISABLED)
  ...
  bgmirrorbutton = Button(image = bgmirrorphoto, command = bgmirror, state = DISABLED)
  ...
  bgrotatebutton = Button(image = bgrotatephoto, command = bgrotate, state = DISABLED)
  ...
  originalbgButton = Button(image = resetBG, command = bgtwo, state = DISABLED)
...
##   tba.pack()
  ...
  turtlepic = Button(image = picture, command = lambda: draw(turtleone))
  ...
  originalButton = Button(image = originalTimage, command = lambda: draw(original23), state = DISABLED)
  originalButton.originalTimage = originalTimage
  originalButton.pack(side = "left")

  Sbutton = Button(text = "S", command = savefunc)
